        ...
        {isLoading ? (
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            {campaigns.map((campaign, index) => {
              ...
              return (
                <article
                  key={buildCampaignKey(campaign, index)}
...
function buildCampaignKey(campaign: AttackCampaign, index: number): string {
  const firstTimelineTimestamp = campaign.timeline[0]?.timestamp;
  const lastTimelineTimestamp = campaign.timeline.at(-1)?.timestamp;
  const firstPhaseEventTimestamp = campaign.phases.find((phase) => phase.events.length > 0)?.events[0]?.timestamp;
  const phaseSignature = campaign.phases
    .filter((phase) => phase.events.length > 0)
    .map((phase) => `${phase.phase}:${phase.events.length}`)
    .join("|");

  return [
    campaign.attacker_ip,
    campaign.campaign_name,
    firstTimelineTimestamp ?? firstPhaseEventTimestamp ?? "no-ts",
    lastTimelineTimestamp ?? "no-end",
    phaseSignature || "no-phases",
    campaign.risk_score,
    index,
  ].join("-");
}
