...
import { cn, countHighSeverity } from "@/lib/utils";
...
export function SummaryCards({ result, isLoading, hasResult }: SummaryCardsProps) {
  ...
  const metrics = [
    ...
    { label: "Campaigns", value: campaigns.length, icon: Radar, tone: "text-emerald-200" },
  ];
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-slate-950/45 p-4">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.26em] text-slate-500">Summary insights</p>
          <p className="mt-2 text-sm text-slate-300">Operational roll-up from the latest completed upload.</p>
        </div>
      </div>
      {!isLoading && !hasResult ? (
        <div className="rounded-3xl border border-dashed border-white/10 bg-slate-950/30 px-5 py-10 text-center text-sm leading-6 text-slate-400">
          Summary insights will appear here after the first analyzed upload.
        </div>
      ) : (
